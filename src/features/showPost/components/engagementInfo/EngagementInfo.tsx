...
import s from './EngagementInfo.module.scss'
...
export const EngagementInfo = (props: Props) => {
  const { postId, postData, className, ...rest } = props
  const { data } = useGetMeQuery()
  const notIsAuth = !data
  const isAuth = !!data
  const { isFetching } = useGetPostDataQuery(postId, { skip: !!postData })

  const styles = clsx(s.engagement_info, className, {
    [s.not_is_auth]: notIsAuth,
  })
